...
| D012 | Accepted — P03 | Use the continuous Wyman/Sloan/Shirley analytic CIE 1931 fit, XYZ-to-linear-sRGB conversion, neutral-lift spectral gamut mapping, fixed per-look exposure, Three.js ACES filmic tone mapping, and one final sRGB transfer. | Implementation owner; creative approval through P03 review | Preserves wavelength order, HDR detail, neutral whites, and viewport/capture parity. Numerical tail chromaticity is held at the nearest reliable endpoint while visible energy fades. See `adr/0002-linear-spectral-output.md`. |
