...
| D012 | Accepted — P03, amended | Use the continuous Wyman/Sloan/Shirley analytic CIE 1931 fit, hard-clamp authored emission to 470–620 nm, reserve 470 nm blue for life and 620 nm red for death, then use XYZ-to-linear-sRGB conversion, neutral-lift gamut mapping, 1.32× source chroma, per-look saturation (+0.18 Porcelain), fixed exposure, Three.js ACES filmic tone mapping, and one final sRGB transfer. | Creative owner + implementation owner | Preserves an unambiguous life/death color axis while giving causal emission a denser second grade; neutral architecture remains neutral and viewport/capture parity stays explicit. See the 2026-08-07 amendments in `adr/0002-linear-spectral-output.md`. |
| D019 | Accepted — P07 | Implement environmental influence as typed independent channels on stable-ID sources, evaluated in stable source-ID order through a dense uniform grid, combined with a per-sign saturating union, and differentiated analytically. | Implementation owner | Declaration order cannot reach a sampled value or a tick hash; food and kill stay separate through metabolism; the analytic gradient keeps full-population sampling inside the P02 tick budget where the contract's central difference did not. See `adr/0003-signed-field-api.md`. |
